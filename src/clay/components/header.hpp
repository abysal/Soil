...
public:
    HeaderBar(
        ...
        : button_store(std::make_tuple(std::move(ButtonElement{std::move(types)}
          )...)),
    ...
    void render(ConfigType &color_config) noexcept {
        new_element(this->host_config, [&] {
            this->for_each_element([&](auto &element) {
                Clay_ElementDeclaration decl = this->build_full_decl(element);
...
private:
    ...
    Clay_ElementDeclaration build_full_decl(ButtonElement<T> &t) noexcept {
        Clay_ElementDeclaration decl = {
            ...
            .cornerRadius    = {this->button_config.rounding, this->button_config.rounding, this->button_config.rounding, this->button_config.rounding}
        };

        RenderInformation render = {.is_hovered = Clay_PointerOver(decl.id)};
