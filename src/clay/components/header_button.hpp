...
public:
    void render(ConfigType &, const RenderInformation&) noexcept {
        const auto clicked =
            this->config.render_button(this->render_text, this->ele_id, std::nullopt, true);
    ...
    std::optional<ClayElementDeclarationPartial> custom_declaration(const RenderInformation& render) const noexcept {
        const auto info = this->config.create_decleration(render.is_hovered, this->ele_id);
        return ClayElementDeclarationPartial{info};
    } 
    
    BaseButton(
        ButtonConfig config, const std::string &id, std::string &&render_text
    ) noexcept
        : config(config), render_id(id), render_text(std::move(render_text)) {
