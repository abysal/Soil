...
namespace soil {
    ...
    void FilesystemTree::deep_index() noexcept {
        ...
        Alloc base_allocator{std::observer_ptr<std::pmr::memory_resource>(backup_alloc), memory};
    ...
    void FilesystemTree::index() {
        ...
        std::span<u8> memory = std::span<u8>{
            (u8 *)std::aligned_alloc(std::alignment_of_v<std::max_align_t>, allocation_size), allocation_size
        };
        ...
        const auto single_layer = [&](std::observer_ptr<FileNode> parent_node, pmr::vector<FolderVisiter> &out_vector,
            ...
            for (const auto &x : root_iter) {
                if (x.is_regular_file()) {
                    ...
                    std::string name = x.path().filename();
                    ...
                    out_vector.emplace_back(FolderVisiter{.insert_target = parent, .folder_location = full_path});
                }
            }
        };
