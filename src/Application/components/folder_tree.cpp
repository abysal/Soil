...
void FolderTree::render(const FolderTreeConfig &config) noexcept {
    ...
    const std::function<void(FileNode &)> render_layer = std::function([&](FileNode &base) {
        for (const auto &node : base.as_folder().children) {
            ...
            new_element({.layout = {.layoutDirection = CLAY_TOP_TO_BOTTOM}}, [&] {
                Color color = Clay_Hovered() ? config.background_color * (1. + config.on_hover_lighten_amount)
                ...
                new_element(
                    ...
                    [&] {
                        new_element(
                            {.layout = {.sizing = {.width = (f32)offset, .height = (f32)config.node_height}}}, [&] {}
                        );
